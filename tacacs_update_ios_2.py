...
'''
ise_username = 'svc-spectrum'
ise_password = "2(8Br4qceJW3m8')>e8lT"
local_secret = "2(8Br4qceJW3m8')>e8lT"
...
def single_device_operations(before_username, before_password, local_secret, device):
    ...
    while run:
        ...
        if nd.good_ssh_connection:
            ...
            try:  # Search for tacacs servers
                step = 'Remove tacacs server'
                pattern = re.compile(r'(tacacs.server.*)')
                tacacs_list = re.findall(pattern, nd.running_config)
...
def Main():
    ...
    if not testing:
        single_device_operations_map = partial(
            single_device_operations, ise_username,
        ...
        for device in device_list:
            result = single_device_operations(
                ise_username, ise_password,local_secret,device)
            result_list.append(result)
    df = pd.DataFrame(result_list)
    df.index += 1
    df.to_csv(f'./IOS-APAC-NADC_TACACS_Switch_Report-{datetime.today().strftime("%Y-%m-%d %H_%M_%S")}.csv')
